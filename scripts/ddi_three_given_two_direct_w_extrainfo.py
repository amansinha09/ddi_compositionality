...
#=====================================================
log_file = f"../resultlogs/logger_logits_order_three_goldofthree_given_two_w_info_{model_name.replace('/', '-')}.jsonl"
...
with open(log_file, "a") as f:
    ...
    for ii, (idx,dd,sde) in enumerate(tqdm(zip(indexes,data,data_ades))):
        ...
        allcombs = get_combination(dd, order=2)
        #print('==>', A,B,C, ygold)
        #'''testing
        hint = ''
        for comb in allcombs:
            #print(comb)
            d1,d2 = comb
            search_index = contains_drug_pair(drug_lists, [d1, d2])
            if search_index == -1:
                print([d1, d2], "not found")
                break

            hint = hint + f'{drugid2drugname[d1]},{drugid2drugname[d2]} --> {sideffectid2sideffectname[side_effect_list[search_index]]}\n'
            # ----------------------------------------------------
            # Prompt
            # ----------------------------------------------------
