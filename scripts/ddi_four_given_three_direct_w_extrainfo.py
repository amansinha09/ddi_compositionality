...
def get_combination(drugs, order):
    return [list(pair) for pair in combinations(drugs, order)]

def contains_drug_pair(big_list, target):
...
#=====================================================
log_file = f"../resultlogs/logger_logits_order_four_goldoffour_given_three_w_info_{model_name.replace('/', '-')}.jsonl"
...
with open(log_file, "a") as f:
    ...
    for ii, (idx,dd,sde) in enumerate(tqdm(zip(indexes,data,data_ades))):
        ...
        allcombs = get_combination(dd, order=3)
        #print('==>', A,B,C, ygold)
        #'''testing
        hint = ''
        for comb in allcombs:
            #print(comb)
            d1,d2, d3 = comb
            search_index = contains_drug_pair(drug_lists, [d1, d2, d3])
            if search_index == -1:
                print([d1, d2, d3], "not found")
                break

            hint = hint + f'{drugid2drugname[d1]},{drugid2drugname[d2]},{drugid2drugname[d3]} --> {sideffectid2sideffectname[side_effect_list[search_index]]}\n'
            # ----------------------------------------------------
            # Prompt
            # ----------------------------------------------------
        gold_ade = ygold #sideffectid2sideffectname[candidate_ade_id]
            
        prompt = f"""You are given a combination of drugs and a candidate adverse effect.

    Hint (drug combination --> side effect):
    {hint}
    ...
    Is this candidate adverse effect associated with the given drug combination?
