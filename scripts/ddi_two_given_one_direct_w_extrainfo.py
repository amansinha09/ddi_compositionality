...
#=====================================================
log_file = f"../resultlogs/logger_logits_order_two_goldoftwo_given_one_w_info_{model_name.replace('/', '-')}.jsonl"


with open(log_file, "a") as f:

    for ii, (idx,dd,sde) in enumerate(tqdm(zip(indexes,data,data_ades))):
        search_index = 9999999
        #print(f"\n========== Example {ii} ==========", dd)
        ...
        for comb in allcombs:
            d1 = comb
            search_index = contains_drug_pair(drug_lists, [d1])
            if search_index == -1:
                print(d1, "not found")
                break
            #else:
            #    print(d1, side_effect_list[search_index], "==>", drugid2drugname[d1], sideffectid2sideffectname[side_effect_list[search_index]])
            hint = hint + f"{drugid2drugname[d1]} --> {sideffectid2sideffectname[side_effect_list[search_index]]}\n"
            #candidate_ade = ygold #sideffectid2sideffectname[candidate_ade_id]
